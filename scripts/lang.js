const langArr = {
    "laulustuudio": {
        "ru": "Вокальная студия",
        "en": "Singing studio",
        "ee": "Laulustuudio",
    },
    "singingstudio": {
        "ru": "Вокальная студия",
        "en": "Singing studio",
        "ee": "Laulustuudio",
    },
    "minust": {
        "ru": "Обо мне",
        "en": "About me",
        "ee": "Minust",
    },
    "kuulamismäng": {
        "ru": "Тест-игра",
        "en": "Game test",
        "ee": "Kuulamismäng",
    },
    "tagasaside": {
        "ru": "Отзывы",
        "en": "Reviews",
        "ee": "Tagasaside",
    },
    "kontakt": {
        "ru": "Контакт",
        "en": "Contact",
        "ee": "Kontakt",
    },
    "about": {
        "ru": "Привет!",
        "en": "Hello!",
        "ee": "Tere!",
    },
    "tore": {
        "ru": "Рада, что вы зашли. Познакомимся.",
        "en": "I'm glad you dropped by. Let's get acquainted.",
        "ee": "Tore, et te sisse tulite. Saame tuttavaks.",
    },
    "minunimi": {
        "ru": "Меня зовут Анна Дытына. Я живу, чтобы сделать этот мир красивее.",
        "en": "My name is Anna Dytyna. I live to make this world more beautiful.",
        "ee": "Minu nimi on Anna Dõtõna. Ma elan selle nimel, et muuta see maailm ilusamaks.",
    },
    "endast": {
        "ru": "Немного о себе",
        "en": "A little bit about yourself",
        "ee": "Natuke endast",
    },
    "lõpetasin": {
        "ru": "Закончила Таллиннскую музыкальную школу им. Георга Отса по классу фортепиано и классического пения под руководством преподавателя Марики Ээнсалу.",
        "en": "I graduated from the Tallinn Music School. Georg Ots in piano and classical singing under the guidance of teacher Marika Eensalu.",
        "ee": "Lõpetasin Tallinna Georg Otsa muusikakooli klaveri ja klassikalise laulmise erialal õpetaja Marika Eensalu juures.",
    },
    "olenomandanud": {
        "ru": "Получила степень бакалавра в Эстонской академии музыки и театра по специальности музыковедение и классическое пение (преподаватель Хелен Лепалане).",
        "en": "I received my bachelor's degree from the Estonian Academy of Music and Theatre, majoring in musicology and classical singing (teacher Helen Lepalane).",
        "ee": "Olen omandanud bakalaureusekraadi Eesti Muusika-ja Teatriakadeemias muusikateaduse ja klassikalise laulmise erialal (õpetaja Helen Lepalane).",
    },
    "magistrikraadi": {
        "ru": "А в 2017 году получила степень магистра интерпретационной педагогики по направлению классического пения.",
        "en": "And in 2017 he received a master's degree in interpretive pedagogy in the direction of classical singing.",
        "ee": "Ja sai 2017.aastal magistrikraadi tõlgendavas pedagoogikas klassikalise laulmise suunas.",
    },
    "laulan": {
        "ru": "Я пою в Камерном хоре Эстонской филармонии.",
        "en": "I sing in the Estonian Philharmonic Chamber Choir.",
        "ee": "Laulan Eesti Filharmoonia Kammerkooris.",
    },
    "samuti": {
        "ru": "Мне было бы интересно встретиться с вами, если вы тоже интересуетесь музыкой.",
        "en": "I would be interested in meeting you if you are also interested in music.",
        "ee": "Samuti tahaksin teiega kohtuda, kui olete huvitatud muusikast.",
    },
    "helistage": {
        "ru": "Звоните (тел.: +372 55614084) и поговорим.",
        "en": "Call (tel.: +372 55614084) and we'll talk.",
        "ee": "Helistage (tel: +372 55614084) ja me räägime.",
    },
    "talking": {
        "ru": "Ученики рассказывают...",
        "en": "Students are talking...",
        "ee": "Õpilased räägivad...",
    },
    "spiritual": {
        "ru": "Музыка — самое духовное из искусств. Вы должны находиться на определенном духовном уровне, чтобы преподавать музыку. И у Анны это есть. Вот почему у неё легко учиться. И я не знаю никого другого, с кем было бы так легко.",
        "en": "Music is the most spiritual of the arts. You have to be on a certain spiritual level to teach music. And Anna is there. That's why it's easy to learn from him. And I don't know anyone else with whom it would be so easy.",
        "ee": "Muusika on kunstidest kõige vaimsem. Muusika õpetamiseks peate olema teatud vaimsel tasandil. Ja Anna on seal. Seetõttu on temalt lihtne õppida. Ja ma ei tea kedagi, kellega oleks veel nii lihtne.",
    },
    "important": {
        "ru": "Мне важно, чтобы учитель не кричал, не злился, иначе я не захочу учиться. Анна очень добрая. Это важно для меня!",
        "en": "It is important for me that the teacher does not shout, does not get angry, otherwise I do not want to learn. Anna is very kind. This is important to me!",
        "ee": "Minu jaoks on oluline, et õpetaja ei karjuks, ei vihastaks, muidu ei taha ma õppida. Anna on väga lahke. See on minu jaoks oluline!",
    },
    "relax": {
        "ru": "Я занимаюсь с Анной совсем недавно, но уже успела научиться расслабиться во время пения. Это то, чего мне не хватало.",
        "en": "I've been working with Anna recently, but I've already managed to relax while singing. I really needed it.",
        "ee": "Tegelen hiljuti Anna juures, kuid laulmise ajal on juba õnnestunud lõõgastuda. Mul oli seda väga vaja.",
    },
    "katrin": {
        "ru": "Катрин",
        "en": "Katrin",
        "ee": "Katrin",
    },
    "milana": {
        "ru": "Милана",
        "en": "Milana",
        "ee": "Milana",
    },
    "olga": {
        "ru": "Ольга",
        "en": "Olga",
        "ee": "Olga",
    },
    "message": {
        "ru": "Отправить сообщение",
        "en": "Send a message",
        "ee": "Saada sõnum",
    },
    "form": {
        "ru": "Есть вопросы?",
        "en": "Do you have any questions?",
        "ee": "Kas teil on küsimusi?",
    },
    "submit": {
        "ru": "Отправить",
        "en": "Send",
        "ee": "Saada",
    },
    "question": {
        "ru": "Вопрос:",
        "en": "Question:",
        "ee": "Küsimus:",
    },
    "name": {
        "ru": "Имя",
        "en": "Name",
        "ee": "Nimi",
    },
    "vocals": {
        "ru": "Вокал",
        "en": "Vocals",
        "ee": "Vokaal",
    },
    "solovocals": {
        "ru": "Сольный вокал для взрослых и детей",
        "en": "Solo vocals for adults and children",
        "ee": "Soolovokaal täiskasvanutele ja lastele",
    },
    "trios": {
        "ru": "Пение для взрослых в дуэте и ​​трио",
        "en": "Singing for adults in duets and trios",
        "ee": "Täiskasvanutele laulmine duettides ja triodes",
    },
    "group": {
        "ru": "Для детей (3-6 лет) - пение в группе (до 5 человек)",
        "en": "For children (3-6 years old) - singing in a group (up to 5 people)",
        "ee": "Lastele (3-6-aastased) - laulmine rühmas (kuni 5 inimest)",
    },
    "options": {
        "ru": "Другие возможности",
        "en": "Other options",
        "ee": "Muud võimalused",
    },
    "piano": {
        "ru": "Фортепиано (для начинающих)",
        "en": "Piano (for beginners)",
        "ee": "Klaverimäng (algajatele)",
    },
    "solfeggio": {
        "ru": "Сольфеджио",
        "en": "Solfeggio",
        "ee": "Solfeggio",
    },
    "therapy": {
        "ru": "Дыхательные практики / Терапия.",
        "en": "Breathing practices / Therapy.",
        "ee": "Hingamispraktikad / Teraapia.",
    },
    "": {
        "ru": "",
        "en": "",
        "ee": "",
    },

}
